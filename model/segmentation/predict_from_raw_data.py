...
class nnUNetPredictor(object):
    """
    Inference helper for nnU-Net style segmentation models.
    Supports prediction from files, numpy arrays, and preprocessed iterators,
    with optional TTA mirroring and sliding-window aggregation.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize predictor runtime options.
        Args:
            tile_step_size: overlap step ratio for sliding-window inference
            use_gaussian: whether to use gaussian weighting for tile fusion
            use_mirroring: whether to enable test-time mirroring augmentation
            perform_everything_on_device: keep accumulation buffers on device if possible
            device: inference device
            verbose: print inference logs
            verbose_preprocessing: print preprocessing logs
            allow_tqdm: enable progress bars
        """
        ...
        # CUDA-only fast path for full on-device execution
        if device.type == 'cuda':
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        Args:
            model_training_output_dir: folder containing plans, dataset json, and fold checkpoints
            use_folds: tuple/list of folds or None for auto-detect
            checkpoint_name: checkpoint filename inside each fold folder
        """
        # Auto-detect available folds when user does not specify them
        if use_folds is None:
            use_folds = nnUNetPredictor.auto_detect_available_folds(model_training_output_dir, checkpoint_name)

        # Load training metadata needed for preprocessing and decoding
        ...
        # Collect network weights for all requested folds
        parameters = []
    ...
    # Infer expected input channels and restore network backbone
        num_input_channels = determine_num_input_channels(plans_manager, configuration_manager, dataset_json)
        trainer_class = recursive_find_python_class(join(nnunetv2.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'nnunetv2.training.nnUNetTrainer')

        network = effunet3d_xl(num_classes=39).to(self.device)
        ...
        # Optional compile path for faster inference
    ...
    def manual_initialization(self, network: nn.Module, plans_manager: PlansManager,
        ...
        """
        Manual initialization path used by trainer-side validation.
        """
        # Directly adopt externally created components/state
        self.plans_manager = plans_manager
        self.configuration_manager = configuration_manager
        self.list_of_parameters = parameters
        self.network = network
        ...
        # Respect compile flag only when current wrapper supports it
        allow_compile = True
    ...
    def auto_detect_available_folds(model_training_output_dir, checkpoint_name):
        """
        Discover fold IDs that contain the requested checkpoint file.
        """
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]],
        ...
        """
        Normalize and shard input/output case lists for current prediction worker.
        Returns:
            filtered input lists, output filenames, previous-stage segmentation files
        """
        # Build case list from folder if needed
        ...
        # Derive output filenames from output folder (or use explicit list)
        ...
        # Map previous-stage segmentations for cascaded models
    ...
    def predict_from_files(self,
                           ...
                           part_id: int = 0):
        """
        Main batch prediction entry from input files/folders.
        Best suited for predicting multiple cases in one run.
        """
        # Resolve output folder regardless of output argument style
        if isinstance(output_folder_or_list_of_truncated_output_files, str):
        ...
        # Run predictor and export pipeline
    ...
    def _internal_get_data_iterator_from_lists_of_filenames(self,
                                                            ...
                                                            num_processes: int):
        """
        Create preprocessing iterator from filename lists.
        """
    ...
    def get_data_iterator_from_raw_npy_data(self,
                                            ...
                                            num_processes: int = 3):
        """
        Build preprocessing iterator from in-memory numpy arrays.
        """

        # Normalize all inputs to list-based form
        ...
        # Keep worker count within dataset size
    ...
    def predict_from_list_of_npy_arrays(self,
                                        ...
                                        num_processes_segmentation_export: int = default_num_processes):
        """
        Convenience API: predict directly from numpy arrays.
        """
    ...
    def predict_from_data_iterator(self,
                                   ...
                                   num_processes_segmentation_export: int = default_num_processes):
        """
        Predict from an iterator of preprocessed samples.
        Each sample must contain keys: 'data', 'ofile', 'data_properties'.
        If 'ofile' is None, predictions are returned in memory.
        """
        # Multiprocessing pool for asynchronous resampling/export
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # Support temporary .npy handoff from preprocessing workers
                ...
                # Network inference (returns logits)
            ...
            # Wait for all background export jobs
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        """
        Predict a single in-memory case.
        Note:
            image_properties is expected to contain spacing metadata.
        """
        # Create one-sample preprocessing adapter
        ...
        if output_file_truncated is not None:
            ...
            # Return segmentation (and probabilities if requested)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        """
        Predict logits from preprocessed tensor data.
        For cascaded setups, previous-stage segmentation must already be stacked
        as one-hot channels in the input.
        Returned logits match preprocessed shape and must be converted back to
        original image space afterwards.
        """
        # Temporarily cap thread usage during inference
        n_threads = torch.get_num_threads()
        torch.set_num_threads(default_num_processes if default_num_processes < n_threads else n_threads)
        with torch.no_grad():
            prediction = None

            # Fold ensemble: run each checkpoint and average predictions
    ...
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        """
        Compute all spatial slices for sliding-window inference.
        """
        slicers = []
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        """
        Predict one patch, optionally with mirroring TTA and average fusion.
        """
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        """
        Run tiled inference and aggregate logits over all sliding windows.
        """
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                # Crop one tile and run model inference
                workon = data[sl][None]
                workon = workon.to(self.device, non_blocking=False)

                prediction = self._internal_maybe_mirror_and_predict(workon)[0].to(results_device)

                # Accumulate logits and normalization weights
                predicted_logits[sl] += (prediction * gaussian if self.use_gaussian else prediction)
                n_predictions[sl[1:]] += (gaussian if self.use_gaussian else 1)

            # Normalize by number/weight of overlapping predictions
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        """
        Public sliding-window inference API for a single preprocessed tensor.
        """
...
def predict_entry_point_modelfolder():
    """
    CLI entry point for prediction from an explicit trained model folder.
    """
    import argparse
...
def predict_entry_point():
    """
    Standard nnU-Net style CLI entry point using dataset/config identifiers.
    """
    import argparse
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 'you want to manually specify a folder containing a trained nnU-Net '
