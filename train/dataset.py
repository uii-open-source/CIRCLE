...
def crop_image(image, center, spacing, crop_size, axes, default_value=-1024):
    ...
    crop_size = np.asarray(crop_size, dtype=np.float64)
...
def prepare_image(img_path, crop_center, augmentation=False):
    """
...
def load_report_text(csv_file):
    """
    Load report text (finding + impression) from CSV.
    Args:
        csv_file: path to CSV containing image_name, finding, and impression columns.
    Returns:
        dict: image_name -> [finding, impression]
    """
...
def load_vqa(vqa_json):
    """
    Load VQA question-answer pairs from a JSON file.
    Args:
        vqa_json: path to JSON file mapping image_name to a list of {question, answer} dicts.
    Returns:
        dict: image_name -> list of VQA dicts
    """
    with open(vqa_json, 'r') as f:
        image_to_vqas = json.load(f)
    return image_to_vqas


def label_to_str(labels, label_names):
    """
    Convert a binary label list to a list of positive label name strings.
    Args:
        labels: list of binary values (0 or 1) of length N.
        label_names: list of label name strings of length N.
    Returns:
        list of str: names of positive (value == 1) labels
    """
    label_str = []
...
class CIRCLEDataset(Dataset):
    ...
    def prepare_samples(self):
        ...
        for name in image_names:
            ...
            lung_center = self.image_to_center[name]  # might be replaced with actual lung center if needed
...
class CIRCLEVqaDataset(Dataset):
    """
    PyTorch Dataset for CIRCLE VQA task.
    Loads CT images, lung center coordinates, and VQA question-answer pairs.
    Each sample corresponds to one (image, question, answer) triplet.
    """
    def __init__(self, data_folder, lung_center_csv, vqa_json):
        self.data_folder = data_folder
        # Load lung center coordinates for each image
        self.image_to_center = get_lung_center(lung_center_csv)
        # Load vqa for each image
        self.image_to_vqas = load_vqa(vqa_json)
        # Prepare list of samples for __getitem__
        self.samples = self.prepare_samples()
        print('--------------- num of sample: {} ----------------'.format(len(self.samples)))
    ...
    def prepare_samples(self):
        """
        Prepare list of samples, expanding each image's VQA list into individual entries.
        Returns:
            list of tuples: (image_path, lung_center, vqa_dict)
        """
        samples = []
        image_names = sorted(self.image_to_vqas.keys())
        img_dir = self.data_folder
        for name in image_names:
            if name not in self.image_to_center:
                continue
            image_path = os.path.join(img_dir, name, 'CT.nii.gz')
            if not os.path.exists(image_path):
                print('{} not exist'.format(image_path))
                continue
            vqa_list = self.image_to_vqas[name]
            lung_center = self.image_to_center[name]  # might be replaced with actual lung center if needed
            for vqa in vqa_list:
                samples.append((image_path, lung_center, vqa))
        return samples
    ...
    def __getitem__(self, index):
        ...
        Returns:
            ...
            answer: answer string
        """
        image_path, lung_center, vqa = self.samples[index]
        image_tensor = prepare_image(image_path, lung_center)
...
class CIRCLEClassificationDataset(Dataset):
    """
    PyTorch Dataset for clinical classification task.
    Reads image paths, lung center coordinates, and class labels from a CSV file.
    Supports incremental loading via read_data_csv.
    """
    def __init__(self):
        # Initialize empty sample list and counter
        self.data_list = []
        self.data_num = 0
        
    def read_data_csv(self, data_csv):
        """
        Read image records from a CSV file and append them to the dataset.
        Args:
            data_csv: path to CSV containing image_path, lung_center_world_x/y/z, and label columns.
        """
        ...
        for i in range(df.shape[0]):
            ...
            if not os.path.exists(image_path):
                print('{} not exist'.format(image_path))
                continue
            # Read lung center coordinates as a numpy array
            lung_center = np.array([df.loc[i, 'lung_center_world_x'],
                                    df.loc[i, 'lung_center_world_y'],
                                    df.loc[i, 'lung_center_world_z']])
            raw_label = df.loc[i, 'label']
            self.data_list.append([image_path, lung_center, raw_label])
    ...
    def __getitem__(self, index):
        ...
        Returns:
            image_tensor: torch.FloatTensor (1, D, H, W)
            label_tensor: torch.LongTensor scalar class label
        """
        image_path, lung_center, label = self.data_list[index]
        image_tensor = prepare_image(image_path, lung_center)
