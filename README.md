...
| -------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Finding**    | 左上肺舌段见类圆形肿块影，大小约65×56mm，病灶周围见斑片模糊影及条片状实变影；两肺少许斑点条索影，左下肺少许实变影，所见各支气管腔通畅，纵隔见轻度肿大淋巴结，左侧胸腔内少量积液。 | A round-like mass measuring approximately 65 × 56 mm is observed in the lingular segment of the left upper lobe. Patchy opacities and streaky consolidation are seen around the lesion. A few spotted and streaky shadows are noted in both lungs, along with areas of consolidation in the left lower lobe. The bronchial airways are patent. Mildly enlarged lymph nodes are present in the mediastinum, accompanied by a small amount of pleural effusion on the left side. |
...
- `model/prompt_seg/`: CIRCLE-based text-guided lesion segmentation model and prompt decoder.
- `train/`: training entry scripts and trainer/dataset utilities.
- `train/precompute_text_embeddings.py`: offline prompt-embedding generation for lesion segmentation.
- `train/run_train_rex.py`: prompt-based lesion segmentation training entry point.
- `test/`: inference/evaluation scripts, including `run_prompt_seg.py` for prompt-based lesion segmentation.
...
### 10. Prompt-based Lesion Segmentation with CIRCLE (Prompt-Seg)

CIRCLE-Prompt-Seg adapts the VoxTell text-prompted volumetric segmentation framework
to the ReXGroundingCT benchmark. Given a raw non-contrast CT volume and a free-text
radiology-style prompt describing a target finding (e.g. "pulmonary nodule in right
upper lobe measuring 1.2 cm"), the model outputs a binary segmentation mask that
spatially localizes the finding.
...
2. **Frozen text encoder** — Qwen3-Embedding-4B computes prompt
   embeddings (`text_emb` pooled vector + `text_tokens` per-token last hidden state +
   `text_mask`). Text embeddings are produced once offline and reused during
   training; the text encoder is not optimized with the segmentation model.
...
Prompt-Seg expects CT volumes, per-finding ground-truth masks, pre-computed text
embeddings, a lung-centre CSV, and a split manifest. The ground-truth masks come from
the annotated dataset; only the text embeddings are generated by the one-time
pre-processing step in §10.2.
...
| `text_emb_4b/*_<idx>.pt` | **Pre-computed** Qwen3-Embedding-4B prompt embeddings written by `train/precompute_text_embeddings.py`. Each `.pt` is a 3-key dict: <br>• `text_emb` `(2560,)` pooled global vector.<br>• `text_tokens` `(L, 2560)` per-token last hidden state.<br>• `text_mask` `(L,)` attention mask (`1 = valid`, `0 = pad`). |
...
>
> The `name` value in the dataset JSON must also match `image_name` in
> `lung_center.csv`. `RexGroundingCTDataset` skips entries whose CT, GT mask, or
> embedding file is missing, so check the logged sample count before training.
...
- `--model_name_or_path`: Required local folder or Hugging Face repo id for
  **Qwen3-Embedding-4B**. Pass the same model to `test/run_prompt_seg.py` for
  runtime prompt encoding (§10.4).
...
resume_checkpoint  = ''  # or a previously saved VisionEncoder.<steps>.pt
...
Then launch distributed training with 🤗 Accelerate (and DeepSpeed if it is
enabled in your Accelerate configuration):
...
Key settings:
- `model_text_dim`: Must be `2560`, matching Qwen3-Embedding-4B embeddings in
  `dataset_embed_dir`.
- `vision_pretrained`: CIRCLE image-encoder checkpoint used for initialization.
- `freeze_image_encoder`: Set to `False` for full fine-tuning or `True` for the
  encoder-freezing ablation.
- `resume_checkpoint`: Optional Prompt-Seg checkpoint from an earlier run.

Output:
- Model checkpoints named `VisionEncoder.<steps>.pt` under `results_folder`.
...
- `--image_name`: Lookup key in `--center_csv` for this case. It should match the
  CSV identifier; if the CSV stores the stem only, a supplied `.nii.gz` suffix is
...
Output:
- A binary `uint8` NIfTI mask at `output_mask_path`, resampled to the input CT's
  original size, spacing, origin, and direction.
